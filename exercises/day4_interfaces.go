...
//
// Add — assign the next ID, increment nextID, append, return the task
// func (s *MemoryStore) Add(task Task) Task {
// 	task.ID = s.nextID
// 	s.nextID++
...
func main() {
	...
	// if found, ok := store.GetByID(t3.ID); ok {
	// 	found.Done = true
	// }
	// printSummary(store)
